...
(async () => {
  ...
  try {
    ...
    const tooltipButtons = await window.evaluate(() => {
      return Array.from(document.querySelectorAll('button[aria-describedby]')).map((button, idx) => ({
        idx,
        text: button.textContent?.trim(),
        ariaDescribedBy: button.getAttribute('aria-describedby'),
        className: button.className,
        html: button.outerHTML.slice(0, 400),
        svgPaths: Array.from(button.querySelectorAll('svg path')).map(p => p.getAttribute('d')),
      }));
    });
    console.log(JSON.stringify(tooltipButtons, null, 2));
  } finally {
