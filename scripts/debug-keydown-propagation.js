...
(async () => {
  ...
  try {
    ...
    await window.evaluate(() => {
      ...
      window.__keydownSeen = false;
      window.addEventListener('keydown', (e) => {
        console.log('CUSTOM KEYDOWN', e.key, e.code, e.ctrlKey, e.defaultPrevented);
        window.__keydownSeen = true;
      }, true);
    ...
    const after = await window.evaluate(() => ({ called: window.__saved, keydownSeen: window.__keydownSeen }));
