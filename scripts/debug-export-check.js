...
(async () => {
  ...
  try {
    ...
    fs.writeFileSync(
      settingsPath,
      'description=Playwright export test\ngame=flare-game\nversion=1.14\nengine_version_min=1.13.01\n',
      'utf8'
    );
    const mapData = {
      name: projectName,
    ...
    console.log('buttons before open:', await window.evaluate(() => Array.from(document.querySelectorAll('button')).map((b) => b.textContent?.trim())));

    const openBtn = await window.locator('button', { hasText: 'Open Existing Project' }).first();
    await openBtn.click();
    console.log('clicked open existing');

    await window.waitForSelector('canvas', { timeout: 15000 });
    console.log('canvas visible');

    console.log('buttons after open:', await window.evaluate(() => Array.from(document.querySelectorAll('button')).map((b) => b.textContent?.trim())));
    console.log('menu button check after open', await window.evaluate(() => {
    ...
    await window.evaluate(() => {
      const button = document.querySelector('button:has(svg.lucide-menu)');
      if (button instanceof HTMLElement) button.click();
    });
    await window.waitForTimeout(500);
    console.log('menu open, button texts after menu open', await window.evaluate(() => Array.from(document.querySelectorAll('button')).map((b) => b.textContent?.trim())));
    console.log('export button after menu open', await window.evaluate(() => {
    ...
    console.log('text fragment:', modalText.slice(0, 200));
    await window.screenshot({ path: 'scripts/debug-export-check-after.png' });
  } finally {
