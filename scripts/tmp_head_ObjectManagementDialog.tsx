...
const ObjectManagementDialog = ({
  ...
  const dialogContent = (
    ...
    {objectValidationErrors.length > 0 && (
        ...
        {/* Role-specific compact options removed â€” settings are now inline with toggles */}
            ...
            <div className="flex gap-2 items-center">
              ...
              {getEditingObjectProperty('anim_image_width', '') && (
                <span className="text-[10px] text-muted-foreground">
                  {getEditingObjectProperty('anim_image_width', '?')}Ã—{getEditingObjectProperty('anim_image_height', '?')} &bull; {getEditingObjectProperty('anim_frames', '?')} frame{getEditingObjectProperty('anim_frames', '1') !== '1' ? 's' : ''}
                </span>
              )}
            </div>
                    ...
                    {itemsList.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-xs text-muted-foreground mt-1 truncate">{item.category || 'Unknown'} Â· {item.role}</div>
                        </button>
                      );
                    })}
                  </div>
