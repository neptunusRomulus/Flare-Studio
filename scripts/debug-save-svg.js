...
(async () => {
  ...
  try {
    ...
    const buttonsWithSvg = await window.evaluate(() => {
      return Array.from(document.querySelectorAll('button')).flatMap((button, idx) => {
        const svg = button.querySelector('svg');
        if (!svg) return [];
        const paths = Array.from(svg.querySelectorAll('path')).map((path) => path.getAttribute('d'));
        return [{ idx, ariaLabel: button.getAttribute('aria-label'), title: button.getAttribute('title'), text: button.textContent?.trim(), paths }];
      });
    });
    console.log(JSON.stringify(buttonsWithSvg, null, 2));
  } finally {
